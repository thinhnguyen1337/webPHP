            ...
            <div class="login-form">
                <form action="" method="POST">
                    ...
                    <table style="text-align:left;">
                        ...
                        <tr>
                            <th>Role:</th>
                            <td><input name="role" type="radio" value="User" <?php if($row['role'] == '0') echo "checked" ?>>
                                <label for="role">User</label>
                             <input name="role" type="radio" value="Admin" <?php if($row['role'] == '1') echo "checked" ?>>
...
<?php  
    if(isset($_POST['submit']) && validate_name($_POST['name']) && validate_phone($_POST['phone_number']) && $_POST['password'] == $_POST['repassword']) {
        ...
        elseif($password == '') {
            $sql = "UPDATE `login` SET `name`='$name',`sex`='$sex',`phone`='$phone_number',`birthday`='$birthday',`role`='$role' WHERE username='$username'";
        ...
        elseif($password != '') {
            ...
            $sql = "UPDATE `login` SET `password`='$password',`name`='$name',`sex`='$sex',`phone`='$phone_number',`birthday`='$birthday',`role`='$role' WHERE username='$username'";
            $query = mysqli_query($conn, $sql);
        ?>
            <br><p style="position: relative;left:120px;"><?php echo "Change infomation success!"; ?></p>
        <?php
